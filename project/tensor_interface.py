...
def st_visualize_tensor(
    ...
    # Ensure tensor has at least 3 dimensions for visualization
    while len(tensor.shape) < 3:
        tensor = tensor.unsqueeze(-1)  # Add a dimension of size 1
    ...
    for i, index in enumerate(highlighted_index):
        highlighted_position += index * strides[i]
